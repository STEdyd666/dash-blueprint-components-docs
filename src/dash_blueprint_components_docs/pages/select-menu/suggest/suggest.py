...
component = html.Div(
    children=[
        html.H1(parse_docstring(dbpc.Suggest)['name'], className='bp5-heading docs-title'),
        html.P(parse_docstring(dbpc.Suggest)['description']),
        html.Div(
            children=[
                html.Div(
                    children=[
                ...
                html.Div(
                    children=[
                        ...
                        dbpc.Switch(
                            id='suggest-resetclose',
                            children='Reset on close',
                        ),
                        dbpc.Switch(
                            id='suggest-resetquery',
                            children='Reset on query',
                        ),
                        dbpc.Switch(
                            id='suggest-resetselect',
                            children='Reset on select',
                        ),
                        dbpc.Switch(
                            id='suggest-useinitial',
                        ...
                        dbpc.Switch(
                            id='suggest-matchwidth',
                            children='Match target width'
                        ),
                        dbpc.Switch(
                            id='suggest-minimalpopover',
        ...
        html.Table(
            children=[
                ...
                html.Tbody(
                    children=get_tablebody_from_props(parse_docstring(dbpc.Suggest)['props'])
                )
            ],
...
@callback(
    Output('suggest', 'closeOnSelect'),
    Input('suggest-closeselect', 'checked')  
)
def change(value):
...
@callback(
    Output('suggest', 'openOnKeyDown'),
    Input('suggest-keydown', 'checked')  
)
def change(value):
...
@callback(
    Output('suggest', 'resetOnClose'),
    Input('suggest-resetclose', 'checked')  
)
def change(value):
...
@callback(
    Output('suggest', 'resetOnQuery'),
    Input('suggest-resetquery', 'checked')  
)
def change(value):
...
@callback(
    Output('suggest', 'resetOnSelect'),
    Input('suggest-resetselect', 'checked')  
)
def change(value):
...
@callback(
    Output('suggest', 'initialContent'),
    Input('suggest-useinitial', 'checked')  
)
def change(value):
    if value:
        return html.Ul(
...
@callback(
    Output('suggest', 'disabled'),
    Input('suggest-disabled', 'checked')  
)
def change(value):
...
@callback(
    Output('suggest', 'matchTargetWidth'),
    Input('suggest-matchwidth', 'checked')  
)
def change(value):
...
@callback(
    Output('suggest', 'minimal'),
    Input('suggest-minimalpopover', 'checked')  
)
def change(value):
