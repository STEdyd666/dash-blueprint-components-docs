...
component = html.Div(
    children=[
        html.H1(parse_docstring(dbpc.Omnibar)['name'], className='bp5-heading docs-title'),
        html.P(parse_docstring(dbpc.Omnibar)['description']),
        html.Div(
            children=[
                html.Div(
                    children=[
                ...
                html.Div(
                    children=[
                        html.H4('Props'),
                        dbpc.Switch(
                            id='omnibar-resetselect',
                            children='Reset on select',
                            defaultChecked=True,
                        ),
                        html.H4('Overlay props'),
                        dbpc.Switch(
                            id='omnibar-backdrop',
        ...
        html.Table(
            children=[
                ...
                html.Tbody(
                    children=get_tablebody_from_props(parse_docstring(dbpc.Omnibar)['props'])
                )
            ],
...
@callback(
    Output('omnibar', 'isOpen'),
    Input('open-omnibar', 'n_clicks')
)
def change(n_clicks):
...
@callback(
    Output('omnibar', 'resetOnSelect'),
    Input('omnibar-resetselect', 'checked')
)
def change(value):
...
@callback(
    Output('omnibar', 'overlayHasBackdrop'),
    Input('omnibar-backdrop', 'checked')
)
def change(value):
...
@callback(
    Output('omnibar-toaster', 'toasts'),
    Input('omnibar', 'selectedItem')
)
def change(value):
    ...
    toasts=[
        dbpc.Toast(
            message=f"You selected {value['label']}"
        )
    ]
