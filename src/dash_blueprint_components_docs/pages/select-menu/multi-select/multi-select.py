...
component = html.Div(
    children=[
        html.H1(parse_docstring(dbpc.MultiSelect)['name'], className='bp5-heading docs-title'),
        html.P(parse_docstring(dbpc.MultiSelect)['description']),
        html.Div(
            children=[
                html.Div(
                    children=[
                ...
                html.Div(
                    children=[
                        html.H4('Props'),
                        dbpc.Switch(
                            id='multiselect-keydown',
                            children='Open popover on key down'
                        ),
                        dbpc.Switch(
                            id='multiselect-resetquery',
                            children='Reset query on select',
                            defaultChecked=True
                        ),
                        dbpc.Switch(
                            id='multiselect-useinitial',
                            children='Use initial content'
                        ),
                        dbpc.Switch(
                            id='multiselect-showclear',
                            children='Show clear button',
                            defaultChecked=True
                        ),
                        ...
                        html.H4('Appearance props'),
                        dbpc.Switch(
                            id='multiselect-minimaltag',
                            children='Minimal tag style'
                        ),
                        dbpc.Switch(
                            id='multiselect-tagintents',
                            children='Cycle through tag intents'
                        ),
        ...
        html.Table(
            children=[
                ...
                html.Tbody(
                    children=get_tablebody_from_props(parse_docstring(dbpc.MultiSelect)['props'])
                )
            ],
...
@callback(
    Output('multiselect', 'openOnKeyDown'),
    Input('multiselect-keydown', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect', 'resetOnSelect'),
    Input('multiselect-resetquery', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect', 'initialContent'),
    Input('multiselect-useinitial', 'checked')  
)
def change(value):
    if value:
        return dbpc.MenuItem(
            disabled=True,
...
@callback(
    Output('multiselect', 'showClearButton'),
    Input('multiselect-showclear', 'checked')  
)
def change(value):
...
@callback(
    ...
    Input('multiselect-disabled', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect', 'fill'),
    Input('multiselect-fill', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect', 'tagMinimal'),
    Input('multiselect-minimaltag', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect', 'tagIntents'),
    Input('multiselect-tagintents', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect', 'matchTargetWidth'),
    Input('multiselect-matchwidth', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect', 'minimal'),
    Input('multiselect-minimalpopover', 'checked')  
)
def change(value):
...
@callback(
    Output('multiselect-source', 'href'),
    Input('navbar', 'route')
)
def sethref(route):
